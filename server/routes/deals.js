...
export default function (io) {
    ...
    // DELETE /api/deals/:id — Cancel and delete a deal
    router.delete("/:id", auth, async (req, res) => {
        try {
            ...
            // Safe extraction of buyer and seller ID strings (in case they are populated or unpopulated)
            const buyerIdStr = deal.buyer_id?._id?.toString() || deal.buyer_id?.toString();
            const sellerIdStr = deal.seller_id?._id?.toString() || deal.seller_id?.toString();

            // Log for debugging
            try {
                fs.appendFileSync("d:\\codelitehacks\\server\\deal_delete_debug.log", 
                    `[${new Date().toISOString()}] DELETE ID: ${req.params.id}\n` +
                    `  - req.user.id (JWT): "${userId}"\n` +
                    `  - deal.buyer_id: "${deal.buyer_id}" (parsed: "${buyerIdStr}")\n` +
                    `  - deal.seller_id: "${deal.seller_id}" (parsed: "${sellerIdStr}")\n`
                );
            } catch (logErr) {
                console.error("Debug log write failed:", logErr);
            }

            // Ensure the user is authorized to delete the deal (either buyer or seller)
            if (buyerIdStr !== userId && sellerIdStr !== userId) {
                return res.status(403).json({ 
                    message: `You are not authorized to cancel this deal. Your ID is ${userId}, but the deal belongs to Buyer: ${buyerIdStr} and Seller: ${sellerIdStr}.` 
                });
            }

            await Deal.findByIdAndDelete(req.params.id);

            // Notify both parties of the deletion via socket
            const payload = { message: "Deal has been cancelled and removed.", dealId: req.params.id };
            if (buyerIdStr) io.to(buyerIdStr).emit("deal_deleted", payload);
            if (sellerIdStr) io.to(sellerIdStr).emit("deal_deleted", payload);

            res.status(200).json({ message: "Deal successfully cancelled and removed.", dealId: req.params.id });
        } catch (err) {
            console.error(err);
            res.status(500).json({ message: "Server error deleting deal: " + err.message });
        }
    });
